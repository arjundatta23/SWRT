...
def do_single_freq(per):
    ...
    # medium 1
    oobj1 = ein.energy_integrals_ray(effile_is, per, dep_pts_is, ishif)
    ...
    # medium 2
    oobj2 = ein.energy_integrals_ray(effile_ts, per, dep_pts_ts, tshif)
    	...
    	if alterwhich==1:
    		...
    		if len(to_add)>0:
    			...
    			if len(z2) != len(z1) or len(phi2) != len(phi1):
    				raise SystemExit('Problem with depth sampling of eigenfunctions from the 2 media')
    			return phi1
    ...
    b1_med1=list(range(len(Nmed1))); b2_med1=list(range(len(Nmed1))); b4_med1=list(range(len(Nmed1)))
    b1_med2=list(range(len(Nmed2))); b2_med2=list(range(len(Nmed2))); b4_med2=list(range(len(Nmed2)))

    psixx_med1=list(range(len(Nmed1)))
    psixx_med2=list(range(len(Nmed2)))
    ...
    try:
        assert np.count_nonzero(rcoef) == len(Nmed1)
        assert np.count_nonzero(tcoef) == len(Nmed2)
    except AssertionError:
        raise SystemExit('You have a problem. Quitting.')
...
########################################################################
# Get necessary information on model interfaces
########################################################################
